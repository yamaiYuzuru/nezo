    ...
    if (guilds[msg.guild.id].english === true) {
        ...
        if(msg.member.permissions.has("KICK_MEMBERS")){
            if(msg.guild.me.permissions.has("KICK_MEMBERS")){
                ...
                bantyp.kick(reason).then(function(){
                    bannedembed.setAuthor(msg.author.tag, msg.author.avatarURL())
                        .setDescription(`${bantyp.user.tag} was kicked by ${msg.author.tag}.`)
                ...
                errorembed.setTitle("I don't have permissions to kick.");
                errorembed.setDescription("If i don't have permissions to kick i can't ban this user.");
                return msg.channel.send(errorembed);
            }
        }else{
            errorembed.setTitle("You don't have permissions to kick!");
            errorembed.setDescription("You dont have the permissions on this guild to kick.");
            return msg.channel.send(errorembed);
        }
        //}
    } else {
        ...
        if(msg.member.permissions.has("KICK_MEMBERS")){
            if(msg.guild.me.permissions.has("KICK_MEMBERS")){
                ...
                bantyp.kick(reason).then(function(){
                    bannedembed.setAuthor(msg.author.tag, msg.author.avatarURL())
                        .setDescription(`${bantyp.user.tag} wurde gekickt von ${msg.author.tag}.`)
                ...
                errorembed.setTitle("Ich habe keine kick rechte");
                errorembed.setDescription("Wenn ich keine kick rechte habe, kann ich diesen Benutzer nicht bannen.");
                return msg.channel.send(errorembed);
            }
        }else{
            errorembed.setTitle("Du hast keine kick rechte");
            errorembed.setDescription("Du hast keine kick rechte auf dem server.");
            return msg.channel.send(errorembed);
        }
    }
    //}
